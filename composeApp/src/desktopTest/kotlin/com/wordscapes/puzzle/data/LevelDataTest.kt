...
class LevelDataTest {
    ...
    companion object {
        ...
        fun loadAssets() {
            ...
            val raw = resolve("src/commonMain/composeResources/files/levels.json").readText()
            val parsed = json.decodeFromString<LevelsFileDto>(raw)
            levels = parsed.levels.map(LevelMapper::toDomain)

            dictionary = resolve("src/commonMain/composeResources/files/dictionary.txt")
                .readLines()
    ...
    fun `all levels parse into domain models`() {
        assertTrue(levels.isNotEmpty(), "expected at least one level")
        assertTrue(levels.size in 10..20, "brief requires 10-20 levels, found ${levels.size}")
    }
    ...
    fun `level ids are unique and ascending`() {
        ...
        assertEquals(ids.sorted(), ids, "levels must be sorted by id")
    }
        ...
        levels.forEach { level ->
            ...
            level.words.forEach { placed ->
                placed.word.groupingBy { it }.eachCount().forEach { (ch, need) ->
                    val have = wheel[ch] ?: 0
                    assertTrue(need <= have, "level ${level.id}: '${placed.word}' needs $need x '$ch', " +
        ...
        levels.forEach { level ->
            level.words.forEach { placed ->
                placed.positions.forEach { pos ->
                    assertTrue(pos.row in 0 until level.gridHeight &&
                            pos.col in 0 until level.gridWidth, "level ${level.id}: '${placed.word}' reaches " +
    ...
    fun `declared dimensions are tight, not padded`() {
        levels.forEach { level ->
            ...
            assertEquals(level.gridHeight - 1, maxRow, "level ${level.id}: declared height ${level.gridHeight} " +
                    "but content ends at row $maxRow")
            assertEquals(level.gridWidth - 1, maxCol, "level ${level.id}: declared width ${level.gridWidth} " +
    ...
    fun `intersections are recorded with both owning words`() {
        levels.forEach { level ->
            ...
            assertTrue(level.words.size < 2 || intersections > 0, "level ${level.id}: a multi-word grid must have at least one " +
                    "intersection, found none")
            level.cells.values.forEach { cell ->
                cell.wordIndices.forEach { idx ->
                    val word = level.words[idx]
                    assertTrue(cell.position in word.positions, "level ${level.id}: cell ${cell.position} claims word " +
        ...
        levels.forEach { level ->
            ...
            listOf(0 to 1, 1 to 0).forEach { (dr, dc) ->
                level.cells.keys.forEach { pos ->
                    ...
                    if (run.length > 1) {
                        assertTrue(run in declared, "level ${level.id}: unintended run '$run' starting at " +
    ...
    fun `grid is a single connected crossword`() {
        levels.forEach { level ->
            ...
            assertEquals(cells.size, seen.size, "level ${level.id}: grid has disconnected islands")
        }
    }
    ...
    fun `every grid word resolves in the shipped dictionary`() {
        levels.forEach { level ->
            level.gridWords.forEach { word ->
                assertTrue(word in dictionary, "level ${level.id}: '$word' is in the grid but missing from " +
                        "dictionary.txt")
            }
        }
    }
    ...
    fun `every level has bonus words available`() {
        levels.forEach { level ->
            ...
            assertTrue(bonus.isNotEmpty(), "level ${level.id}: no bonus words reachable — the bonus-word " +
